...
export default async function PatientDashboardPage({ searchParams }: { searchParams: { confirmed?: string } }) {
  ...
  const patient = await prisma.patient.findUnique({
    ...
    include: {
      appointments: {
        ...
        include: { doctor: { include: { user: true } }, consultationType: true, payment: true, consultation: { include: { notes: true } }, prescriptions: { orderBy: { createdAt: "desc" } } },
      },
    },
  });
  ...
  return (
    <div className="dashboard-frame"><DashboardNav canSwitch={false} /><main className="dashboard-main">
      <div className="dashboard-heading"><div><p className="dashboard-kicker">Your care workspace</p><h1>Good to see you, {session!.user.name?.split(" ")[0]}</h1><p>Keep your care journey organised in one calm place.</p><p className="panel-created">Account created {patient?.createdAt.toLocaleDateString("en-IN") ?? "today"}</p></div><a href="/book-appointment" className="rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white">Book consultation</a></div>
      ...
      <section className="mt-10"><div className="flex items-center justify-between"><h2 className="font-display text-2xl text-brand-900">Previous consultations</h2><a href="/patient/appointments" className="text-sm font-semibold text-brand-700">View all →</a></div>{appointments.filter((appointment) => appointment.consultation?.completedAt || appointment.status === "COMPLETED").length === 0 ? <div className="dashboard-card mt-4"><p className="text-ink/60">Completed consultation history will appear here.</p></div> : <div className="dashboard-list">{appointments.filter((appointment) => appointment.consultation?.completedAt || appointment.status === "COMPLETED").slice(0, 3).map((appointment) => <div className="dashboard-list-item" key={`history-${appointment.id}`}><div><h3>{appointment.consultation?.summary || appointment.consultationType.name}</h3><p>Dr. {appointment.doctor.user.firstName} {appointment.doctor.user.lastName} · {appointment.updatedAt.toLocaleDateString("en-IN")}</p>{appointment.prescriptions[0] && <p><b>Prescribed:</b> {appointment.prescriptions[0].medicine} — {appointment.prescriptions[0].instructions}</p>}</div><span className="rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-800">COMPLETED</span></div>)}</div>}</section>
